...
async function getPeriodInventoryStats() {
    try {
        ...
        // Đảm bảo allHistory và allOperations là mảng
        const safeHistory = Array.isArray(allHistory) ? allHistory : [];
        const safeOperations = Array.isArray(allOperations) ? allOperations : [];
        
        const periodHistory = safeHistory.filter(record => {
            ...
            return recordDate && recordDate >= startDateStr && recordDate <= endDateStr;
        });
        
        const periodOperations = safeOperations.filter(operation => {
            ...
            return operationDate && operationDate >= startDateStr && operationDate <= endDateStr;
        });
        ...
        return {
            ...
            serviceCount: serviceOps.length || 0,
            serviceValue: serviceTotal || 0,
            materialCount: materialOps.length || 0,
            materialValue: materialTotal || 0,
            totalTransactions: periodHistory.length + periodOperations.length,
            totalOperationsValue: totalOperations || 0,
            totalAllValue: importValue + totalOperations,
            periodHistory: periodHistory || [],
            periodOperations: periodOperations || [],
            materialOps: materialOps || [],
            serviceOps: serviceOps || []
        };
        
    } catch (error) {
        console.error('❌ Error getting period stats:', error);
        // TRẢ VỀ OBJECT MẶC ĐỊNH ĐẦY ĐỦ
        return {
            totalImports: 0,
            totalExports: 0,
            importValue: 0,
...
async function renderInventoryTab(container, inventory) {
    ...
    // Đảm bảo periodStats không phải undefined
    ...
    <div class="inventory-content" data-tab="inventory">
        ...
        <div class="date-selector">
            ${safeStats.totalImports > 0 || safeStats.totalExports > 0 ? `
                <div class="period-stats">
                    <h4>📈 Thống kê Kỳ ${formatPeriodDisplay(currentPeriod)}</h4>
                    <div class="stats-grid">
                        <div class="stat-item">
                            <span>📥 Nhập kho:</span>
                            <strong>${safeStats.totalImports} sản phẩm</strong>
                        </div>
                        <div class="stat-item">
                            <span>📤 Xuất kho:</span>
                            <strong>${safeStats.totalExports} sản phẩm</strong>
                        </div>
                        <div class="stat-item">
                            <span>💰 Giá trị nhập:</span>
                            <strong>${formatCurrency(safeStats.importValue)}</strong>
                        </div>
                        <div class="stat-item">
                            <span>📊 Tổng giao dịch:</span>
                            <strong>${safeStats.totalTransactions}</strong>
                        </div>
                    </div>
                </div>
            ` : ''}
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-primary btn-sm" 
                                            ...
                                            data-product-id="${item.id || item.itemId || ''}">
                                        Lịch sử
                                    </button>
                                </td>
                            </tr>
...
async function showProductHistoryPopup(productId) {
    // productId ở đây là itemId của inventory
    ...
    // Lấy lịch sử - kiểm tra cả itemId và id
    const allHistory = await dbGetAll('inventoryHistory');
    const history = allHistory
        .filter(record => record.productId === productId || record.productId === product.id)
...
async function addInventoryRecord(productId, type, quantity, unitPrice, note) {
    ...
    const historyRecord = {
        ...
        quantity: quantity,
        unitPrice: unitPrice,
        totalPrice: type === 'in' ? (quantity * unitPrice) : (quantity * product.averagePrice),
        note: note,
    ...
    console.log('📝 Adding inventory history record:', historyRecord);
    await dbAdd('inventoryHistory', historyRecord);
    
    const updatedProduct = await dbUpdate('inventory', productId, {
    ...
    if (githubSync && githubSync.enabled && githubSync.autoSync) {
        setTimeout(async () => {
            try {
                console.log('☁️ Starting GitHub sync after inventory record...');
                await syncSingleInventoryOperation('inventoryHistory', historyRecord);
                await syncSingleInventoryOperation('inventory', updatedProduct);
                console.log('✅ GitHub sync completed after inventory record');
            } catch (syncError) {
        ...
        if (e.target.matches('[data-action="save-new-product"]')) {
            ...
            try {
                ...
                if (githubSync && githubSync.enabled && githubSync.autoSync) {
                    setTimeout(async () => {
                        try {
                            console.log('☁️ Starting GitHub sync after adding new product...');
                            await syncSingleInventoryOperation('inventory', newProduct);
                            
                            if (quantity > 0) {
                                const historyRecord = await dbGet('inventoryHistory', historyId);
                                if (historyRecord) {
                                    await syncSingleInventoryOperation('inventoryHistory', historyRecord);
                                }
                            }
                            
                            console.log('✅ GitHub sync completed after adding new product');
                        } catch (syncError) {
                            console.error('⚠️ GitHub sync failed:', syncError);
                        }
                    }, 1000);
...
async function saveMaterial() {
    ...
    try {
        ...
        // 3. Tạo bản ghi lịch sử nhập kho
        const historyRecord = {
            id: historyId,
            historyId: historyId,
        ...
        await dbAdd('inventoryHistory', historyRecord);
        console.log('✅ Added inventory history');
        ...
        console.log('📊 Updating inventory:', {
            id: product.id,
            oldQuantity: totalQuantityBefore,
            newQuantity: newTotalQuantity,
            oldValue: totalValueBefore,
            newValue: newTotalValue
        });
        
        const updatedProduct = await dbUpdate('inventory', product.id, {
        ...
        // 5. ĐỒNG BỘ LÊN GITHUB
        if (githubSync && githubSync.enabled && githubSync.autoSync) {
            setTimeout(async () => {
                try {
                    console.log('☁️ Starting GitHub sync after material save...');
                    
                    // Sync từng store riêng biệt
                    await syncSingleInventoryOperation('operations', operationRecord);
                    await syncSingleInventoryOperation('inventoryHistory', historyRecord);
                    await syncSingleInventoryOperation('inventory', updatedProduct);
                    
                    console.log('✅ GitHub sync completed after material save');
                    
                } catch (syncError) {
                    console.error('⚠️ GitHub sync failed:', syncError);
                    // Không hiển thị lỗi để không làm gián đoạn người dùng
                }
            }, 1000);
        }
...
async function saveService() {
    ...
    try {
        ...
        // 2. ĐỒNG BỘ LÊN GITHUB
        if (githubSync && githubSync.enabled && githubSync.autoSync) {
            setTimeout(async () => {
                try {
                    console.log('☁️ Starting GitHub sync after service save...');
                    await syncSingleInventoryOperation('operations', operationRecord);
                    console.log('✅ GitHub sync completed after service save');
                } catch (syncError) {
                    console.error('⚠️ GitHub sync failed:', syncError);
                }
            }, 1000);
...
/**
 * @name syncInventoryToGitHub
 * @description Đồng bộ tất cả dữ liệu kho hàng lên GitHub sau khi cập nhật
 */
async function syncInventoryToGitHub() {
    try {
        console.log('☁️ Syncing inventory data to GitHub...');
        
        // Kiểm tra cấu hình GitHub sync
        if (!githubSync || !githubSync.enabled) {
            console.log('⚠️ GitHub sync disabled');
            return false;
        }
        
        if (!githubSync.token || !githubSync.repo) {
            console.log('⚠️ GitHub config missing');
            return false;
        }
        
        // 1. Đồng bộ inventory (kho hàng)
        const inventoryData = await dbGetAll('inventory');
        if (inventoryData.length > 0) {
            ...
            const inventoryFilename = `${githubSync.dataPath}/inventory.json`;
            const inventorySuccess = await uploadToGitHub(inventoryFilename, cleanInventory);
            
            if (inventorySuccess) {
                console.log(`✅ Uploaded inventory: ${inventoryData.length} records`);
            }
        }
        
        // 2. Đồng bộ inventoryHistory (lịch sử kho)
        const historyData = await dbGetAll('inventoryHistory');
        if (historyData.length > 0) {
            const cleanHistory = historyData.map(item => {
                const { _synced, _source, _lastSync, ...cleanItem } = item;
                return cleanItem;
            });
            
            const historyFilename = `${githubSync.dataPath}/inventoryHistory.json`;
            const historySuccess = await uploadToGitHub(historyFilename, cleanHistory);
            
            if (historySuccess) {
                console.log(`✅ Uploaded inventory history: ${historyData.length} records`);
            }
        }
        
        // 3. Đồng bộ operations (mua sắm vận hành)
        const operationsData = await dbGetAll('operations');
        if (operationsData.length > 0) {
            const cleanOperations = operationsData.map(item => {
                const { _synced, _source, _lastSync, ...cleanItem } = item;
                return cleanItem;
            });
            
            const operationsFilename = `${githubSync.dataPath}/operations.json`;
            const operationsSuccess = await uploadToGitHub(operationsFilename, cleanOperations);
            
            if (operationsSuccess) {
                console.log(`✅ Uploaded operations: ${operationsData.length} records`);
            }
        }
        ...
        const metadataFilename = `${githubSync.dataPath}/inventory_metadata.json`;
        await uploadToGitHub(metadataFilename, metadata);
        
        console.log('✅ Inventory sync to GitHub completed');
        
        // Cập nhật thời gian sync
        localStorage.setItem('last_github_sync_inventory', new Date().toISOString());
        
        // Hiển thị thông báo
        showMessage('✅ Đã đồng bộ dữ liệu kho lên GitHub', 'success');
        return true;
        
    } catch (error) {
        console.error('❌ Error syncing inventory to GitHub:', error);
        showMessage('❌ Lỗi đồng bộ GitHub: ' + error.message, 'error');
        return false;
    }
}

/**
 * @name syncSingleInventoryOperation
 * @description Đồng bộ một thao tác kho cụ thể lên GitHub
 * @param {string} storeName - Tên store (inventory, inventoryHistory, operations)
 * @param {object} data - Dữ liệu cần đồng bộ
 */
async function syncSingleInventoryOperation(storeName, data) {
    try {
        if (!githubSync || !githubSync.enabled || !githubSync.autoSync) {
            return false;
        }
        
        console.log(`☁️ Syncing single ${storeName} operation to GitHub...`);
        
        // 1. Lấy toàn bộ dữ liệu hiện tại
        const allData = await dbGetAll(storeName);
        
        // 2. Clean dữ liệu (loại bỏ các trường internal)
        ...
        if (success) {
            console.log(`✅ Synced ${storeName} to GitHub: ${allData.length} records`);
            
            // Cập nhật trạng thái sync cho tất cả bản ghi
            for (const item of allData) {
                if (item.id || item.productId || item.operationId) {
                    await dbUpdate(storeName, item.id || item.productId || item.operationId, {
                        _synced: true,
        ...
        console.error(`❌ Error syncing single ${storeName} to GitHub:`, error);
        return false;
    }
}
// ============== GLOBAL EXPORTS ==============
...
// Thêm exports mới
window.syncInventoryToGitHub = syncInventoryToGitHub;
window.syncSingleInventoryOperation = syncSingleInventoryOperation;
